...
(async () => {
  ...
  while (true) {
    try {
      ...
      // ---------------------------------------------------------
      // 🔥 IDE RAKTAM BE A RÉGI CIKKEK ÚJRAFELDOLGOZÁSÁT
      // ---------------------------------------------------------
      if (batch.length === 0) {
  ...
  AND (
    ...
    OR s.trend_keywords IS NULL
        ...
        if (oldRows.length > 0) {
          console.log(`🔁 Régi cikkek újrafeldolgozása indul: ${oldRows.length} db`);
          const oldIds = oldRows.map(a => a.id);
          await markStatus(connection, oldIds, "pending");
          continue;
        }

        console.log("😴 Nincs új vagy régi feldolgozatlan cikk. Várakozás...");
